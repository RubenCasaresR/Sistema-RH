<?php
/**
 * Eliminación lógica (soft delete) de un empleado.
 * Cambia activo = 0 en lugar de borrar el registro.
 */
...
try {
    ...
    $stmt = $db->prepare('UPDATE employees SET activo = 0 WHERE id = :id');
    $stmt->execute([':id' => $id]);

    if ($stmt->rowCount() > 0) {
        logAudit('delete', 'employee', $id, json_encode([
            'nombre' => ($emp['nombre'] ?? '') . ' ' . ($emp['apellido_paterno'] ?? ''),
        ]));
        setFlash('success', 'Empleado desactivado correctamente.');
    } else {
        setFlash('danger', 'Empleado no encontrado.');
    }
} catch (PDOException $e) {
    error_log('Error al desactivar empleado: ' . $e->getMessage());
    setFlash('danger', 'Error al desactivar el empleado.');
}
